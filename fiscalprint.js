...
NOME FILE		: fiscalprint.js
DESCRIZIONE		: File principale JavaScript tastiera virtuale
...
14/11/2018	1.1.3
 1. Added console.log entry.


30/10/2019	1.1.4
 1. Added console.log entries.
 
22/06/2020	1.1.5
 1. Added console.log entries.
 
15/10/2020	1.1.6
 1. "Pretty pinked" soap request.
 2. Added console.log entries.
 3. Validated JavaScript.
 
17/11/2020	2.0.0
 1. Added user/password handling (Basic Authentication).

19/11/2020	2.1.0
 1. Added Digest authentication. Extra argument specfies Basic or Digest.

02/12/2021	2.2.0
 1. Commentata riga xhr.setRequestHeader('Origin', 'www.rt-epson.it') che blocca le richieste HTTPS cross-domain.
*/
...
// F I S C A L --- F I S C A L --- F I S C A L --- F I S C A L --- F I S C A L --- F I S C A L --- F I S C A L --- F I S C A L
	...
	*/
	...
	Parameters:
		address		string		The address where fpmate.cgi resides (printer)
		request		string		Request message
		timeout		integer		Response timeout expressed in milliseconds
		callMode	string		async o sync
	...
	*/
	...
	fiscalPrint.prototype.send = function (address, request, timeout, callMode, authMode, username, password)
	{
		console.log("fiscalPrint - " + address + ",\r\n" + request + ", Timeout = " + timeout + ", callMode = " + callMode + ", authMode = " + authMode + ", username = " + username + ", password = " + password + "\r\n");
		timeout = timeout || 0;
		...
		xhr.setRequestHeader('If-Modified-Since', 'Thu, 01 Jan 1970 00:00:00 GMT');
		xhr.setRequestHeader('SOAPAction', '""');
		// xhr.setRequestHeader('Origin', 'www.rt-epson.it');
		if (authMode.toLowerCase() == "basic")
		{
			xhr.setRequestHeader("Authorization", "Basic " + btoa(username + ":" + password));
		}
		if (authMode.toLowerCase() == "digest")
		{
		...
		if (callMode.toLowerCase() == "async")
		{
			...
			{
				console.log("Timed out!!! - fireFiscalErrorEvent.");
		...
		xhr.onreadystatechange = function ()
		{
			...
			if (xhr.readyState == 4)
			{
				console.log("xhr.readyState = 4 xhr.status = " + xhr.status + " xhr.responseText =\r\n" + xhr.responseText);
				if (xhr.status == 200)
				{
					console.log("fireFiscalReceiveEvent");
					fireFiscalReceiveEvent(epos, xhr);					
				}
				else
				{
					console.log("fireFiscalErrorEvent");
					fireFiscalErrorEvent(epos, xhr);					
				}
			}
		};
		...
		if (epos.onreceive)
		{
			...
			if (res.length > 0)
			{
				...
				if (res_add.length > 0)
				{
					...
					tag_names_array = tag_names_list.split(',');					
